...
Public Class AsynchronousClient
    ...
    Public Function ConnectSocket(Server As String, ipPort As String) As Socket
        ...
        Catch ex As Exception
            If SuperDebug Then Log("Error in ConnectSocket displaying IP info with error = " & ex.Message, LogType.LOG_TYPE_ERROR)
        End Try
        Try
            If SuperDebug And Not ipHostInfo Is Nothing Then
            ...
            End If
        Catch ex As Exception
            If g_bDebug Then Log("Error in ConnectSocket displaying IP info with error = " & ex.Message, LogType.LOG_TYPE_ERROR)
        End Try
        Try
            If Not ipHostInfo Is Nothing Then
                ...
                If ip_Address.AddressFamily <> AddressFamily.InterNetwork Then
                    If g_bDebug Then Log("Error in ConnectSocket getting the IPv4 IP address", LogType.LOG_TYPE_ERROR)
                    For index = 0 To UBound(ipHostInfo.AddressList, 1)
                        Dim tempip_Address As IPAddress = ipHostInfo.AddressList(index)
                        Log("ConnectSocket found ipAddress(" & index.ToString & ") = " & tempip_Address.ToString & " and address type = " & tempip_Address.AddressFamily.ToString, LogType.LOG_TYPE_INFO)
                    Next
        ...
        End Try
        Try
            Dim remoteEP As New IPEndPoint(ip_Address, ipPort)
    ...
    Public Function Send(ByVal inData As Byte()) As Boolean
        ...
        If SuperDebug Then Log("Send called with Data = " & Encoding.ASCII.GetString(inData, 0, UBound(inData)), LogType.LOG_TYPE_INFO)
        'If g_bDebug Then Log("Send called with Data = " & Encoding.ASCII.GetString(inData, 0, UBound(inData)), LogType.LOG_TYPE_INFO)
        Send = False
        Try
            If MySocket Is Nothing Then
                sendDone.Set()
